...
class _DoctorPatientProfileScreenState
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar:
          AppBar(title: Text(patient['name'] as String? ?? 'Patient Profile')),
      body: _loading
          ? const LoadingWidget()
          : _error != null
                  ...
                  child: Column(
                      ...
                      children: [
                        ...
                        _ListSection(
                            ...
                            rows: appointments,
                            builder: (row) =>
                                '${Helpers.formatDate(row['appointment_date'] as String? ?? '')} • ${row['status'] ?? ''}'),
                        _ListSection(
                            title: 'Prescriptions',
                            rows: prescriptions,
                            builder: (row) =>
                                '${row['diagnosis'] ?? 'Prescription #${row['id']}'} • ${Helpers.formatDate(row['created_at'] as String? ?? '')}'),
                        _ListSection(
                            title: 'Vitals',
                            rows: vitals,
                            builder: (row) =>
                                'BP ${row['blood_pressure'] ?? '-'} • Pulse ${row['pulse'] ?? '-'} • ${Helpers.formatDate(row['created_at'] as String? ?? '')}'),
                      ]),
                ),
    );
  }
...
class _ListSection extends StatelessWidget {
  ...
  const _ListSection(
      {required this.title, required this.rows, required this.builder});
  ...
  Widget build(BuildContext context) => Padding(
        ...
        child: _Card(
          ...
          if (rows.isEmpty)
            ...
            ...rows.take(8).map((e) => Padding(
                padding: const EdgeInsets.only(bottom: 8),
                child: Text(builder(e as Map<String, dynamic>)))),
        ])),
      );
}
