...
class _DoctorAssistantsScreenState extends State<DoctorAssistantsScreen> {
  ...
  Future<void> _openForm([AssistantModel? assistant]) async {
    final created = await showDialog<AssistantModel>(
        context: context,
        builder: (_) => _AssistantFormDialog(assistant: assistant));
    if (created != null && mounted) {
      final temp = created.tempPassword;
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(
        content: Text(temp == null
            ? 'Assistant saved.'
            : 'Assistant created. Temporary password: $temp'),
                      ...
                      children: _items
                          .map((a) => Container(
                                ...
                                child: ListTile(
                                  ...
                                  subtitle: Text([a.email, a.phone, a.status]
...
class _AssistantFormDialogState extends State<_AssistantFormDialog> {
  late final TextEditingController _name =
      TextEditingController(text: widget.assistant?.name ?? '');
  late final TextEditingController _email =
      TextEditingController(text: widget.assistant?.email ?? '');
  late final TextEditingController _phone =
      TextEditingController(text: widget.assistant?.phone ?? '');
  late Map<String, bool> _perms =
      Map<String, bool>.from(widget.assistant?.permissions ??
          {
  ...
  Widget build(BuildContext context) => AlertDialog(
        title:
            Text(widget.assistant == null ? 'Add Assistant' : 'Edit Assistant'),
        content: SizedBox(
            width: 620,
        ...
        actions: [
          ...
          CustomButton(
              label: 'Save', loading: _saving, onPressed: _save, width: 120),
        ],
      );
}
