...
class _AssistantQueueScreenState extends State<AssistantQueueScreen> {
  ...
  Future<void> _openActions(AppointmentModel a) async {
    await showModalBottomSheet(
        ...
        builder: (_) => _AssistantActionsSheet(
            appointment: a,
            onDone: () {
...
class _AssistantActionsSheet extends StatefulWidget {
  ...
  const _AssistantActionsSheet(
      {required this.appointment, required this.onDone});
...
class _AssistantActionsSheetState extends State<_AssistantActionsSheet> {
  ...
  Future<void> _saveVitals() async {
    ...
    try {
      await AssistantService.addVitals(_vitals.toJson(
  ...
  Future<void> _showHistory() async {
    ...
    try {
      final data =
          await AssistantService.patientHistory(widget.appointment.patientId!);
      if (!mounted) return;
      showDialog(
          context: context,
      ...
      child: SingleChildScrollView(
        child: Column(
            ...
            children: [
              ...
              const Text('Symptoms Draft',
                  style: TextStyle(fontWeight: FontWeight.w800)),
              const SizedBox(height: 10),
              CustomTextField(
                  label: 'Symptoms draft', controller: _symptoms, maxLines: 3),
              const SizedBox(height: 10),
              CustomTextField(
                  label: 'Vitals summary', controller: _summary, maxLines: 2),
              const SizedBox(height: 10),
              CustomButton(
                  label: 'Save Draft',
              ...
              const Divider(height: 32),
              const Text('Upload Report Metadata',
                  style: TextStyle(fontWeight: FontWeight.w800)),
              const SizedBox(height: 10),
              ...
              const SizedBox(height: 10),
              CustomTextField(
                  label: 'Notes', controller: _reportNotes, maxLines: 2),
              const SizedBox(height: 10),
              CustomButton(
