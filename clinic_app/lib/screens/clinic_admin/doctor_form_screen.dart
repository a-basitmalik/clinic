...
class _DoctorFormScreenState extends State<DoctorFormScreen> {
  ...
  void initState() {
    ...
    _fee        = TextEditingController(text: d?.consultationFee?.toStringAsFixed(0) ?? '');
  ...
  @override
  void dispose() {
    for (final c in [_name, _email, _phone, _spec, _qual, _exp, _license, _fee, _startTime, _endTime]) {
      c.dispose();
    }
    super.dispose();
  }
  ...
  Future<void> _save() async {
    ...
    setState(() { _daysError = null; _saving = true; _error = null; });
    ...
    final body = <String, dynamic>{
      ...
      'available_start_time': _startTime.text.trim().isEmpty ? null : _startTime.text.trim(),
      'available_end_time':   _endTime.text.trim().isEmpty ? null : _endTime.text.trim(),
    };
    ...
    await showDialog(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
          const Text('Share these credentials with the doctor. The password is shown only once.', style: TextStyle(fontSize: 13)),
          ...
          : SingleChildScrollView(
              ...
              child: Form(
                ...
                child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  ...
                  CustomTextField(label: 'Full Name *', controller: _name, validator: Validators.required),
                  const SizedBox(height: 12),
                  CustomTextField(label: 'Email *', controller: _email, validator: Validators.email,
                      keyboardType: TextInputType.emailAddress, readOnly: _isEdit),
                  const SizedBox(height: 12),
                  CustomTextField(label: 'Phone', controller: _phone, keyboardType: TextInputType.phone),
                  ...
                  CustomTextField(label: 'Specialization', controller: _spec),
                  const SizedBox(height: 12),
                  CustomTextField(label: 'Qualification', controller: _qual),
                  const SizedBox(height: 12),
                  Row(children: [
                    Expanded(child: CustomTextField(label: 'Experience (years)', controller: _exp, keyboardType: TextInputType.number)),
                    const SizedBox(width: 12),
                    Expanded(child: CustomTextField(label: 'Consultation Fee (PKR)', controller: _fee, keyboardType: TextInputType.number)),
                  ]),
                  const SizedBox(height: 12),
                  CustomTextField(label: 'License Number', controller: _license),
                  const SizedBox(height: 20),
                  ...
                  if (_daysError != null) ...[
                    ...
                    Text(_daysError!, style: const TextStyle(color: AppColors.danger, fontSize: 12)),
                  ],
